...
class Replies {
     ...
     showReplies() {
          ...
          this.replies.items.forEach((item, i) => {
               progress = Math.round((item.replies / this.repliesToComplete) * 100);

               if (item.replies >= this.repliesToComplete) {
                    progress = 100;
                    result.push(`<n>{3AA2D3}${item.date}\t\t    {${this.progressInColor(progress, 81, 46)}}${item.replies} / ${this.repliesToComplete} ✅`); // если норма выполнена то ставиться галочка
               } else {
                    result.push(`<n>{3AA2D3}${item.date}\t\t    {${this.progressInColor(progress, 81, 46)}}${item.replies} / ${this.repliesToComplete}`);
               }
          });
